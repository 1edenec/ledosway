...
TERMINAL_Y=$(( ( (DISPLAY_HEIGHT - TERMINAL_HEIGHT) * 100 ) /103 ))
...
term_launch() {
    ...
    swaymsg -q mark dropdown # Mark the terminal so we can identify it
    swaymsg -q [con_mark=dropdown] move scratchpad
...
term_open() {
    if [ "$(cat $TERMINAL_STATE)" == "closed" ]; then
        ...
        swaymsg -q [con_mark=dropdown] scratchpad show
        swaymsg -q resize set $TERMINAL_WIDTH $TERMINAL_HEIGHT
        swaymsg -q move absolute position $TERMINAL_X $TERMINAL_Y
        swaymsg -q move $STEP_DIRECTION_CL $(( TERMINAL_HEIGHT - 1 ))

        for i in `seq 1 $STEP_SIZE $TERMINAL_HEIGHT`; do
            swaymsg -q [con_mark=dropdown] move $STEP_DIRECTION_OP $STEP_SIZE
            sleep $SLEEP_TIME
        done
...
term_close() {
    if [ "$(cat $TERMINAL_STATE)" == "open" ]; then
        echo "animating" > $TERMINAL_STATE
        swaymsg -q [con_mark=dropdown] focus
        for i in `seq 1 $STEP_SIZE $TERMINAL_HEIGHT`; do
            swaymsg -q [con_mark=dropdown] move $STEP_DIRECTION_CL $STEP_SIZE
            sleep $SLEEP_TIME
        done
        swaymsg -q [con_mark=dropdown] scratchpad show
        echo "closed" > $TERMINAL_STATE
